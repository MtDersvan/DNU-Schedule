...
function createFreqCell(day, order, row) {
  ...
  if ((numerator === -1) ||
    (numerator === 0 && !hasClass(nextRow, 'busy')) ||
    (numerator === 1 && !hasClass(prevRow, 'busy'))) {
...
function onJobTypeChangeHandler(evt) {
  ...
  let numCell = row.querySelector('td.numerator');
  let hiddenDiscipline = numCell.querySelector('[name="discipline"]');
